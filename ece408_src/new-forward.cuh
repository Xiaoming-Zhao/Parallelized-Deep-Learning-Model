    ...
    for (int c = 0; c < C; ++c) {
        ...
        for (int p = 0; p < K; ++p) {
            for (int q = 0; q < K; ++q) {
                if ((h0 + p < X_tile_width) && (w0 + q < X_tile_width)) {
                    // X_shared[h + p, w + q], W_shared[p, q]
                    acc = acc + X_shared[(h0 + p) * X_tile_width + (w0 + q)] * device_k[m * C * K * K + c * K * K + p * K + q];
                }
            }
        }
        __syncthreads();
...
// forward kernel with constant memory
__global__ void forward_shareInput_constKernel_inputRedAtomic(float *y, float *x,
                                                              const int B, const int M, const int C,
                                                              const int H, const int W, const int K)
{
    ...
    */
    ...
    if((n < B) && (m < M) && (h < H_out) && (w < W_out)) {
       ...
       // y[cur_n_out + h * W_out + w] = acc;
       atomicAdd(&y[cur_n_out + h * W_out + w], acc);
       // *(y + cur_n_out + (h) * (W_out) + w) = acc;
   }
}
...
// forward kernel with constant memory
__global__ void forward_shareInput_constKernel_inputRedTree(float *y, float *x,
                                                            const int B, const int M, const int C,
                                                            const int H, const int W, const int K)
{
    ...
    */
    ...
    __shared__ float share_partial_sum[512];
    int idx = c * TILE_WIDTH * TILE_WIDTH + h0 * TILE_WIDTH + w0;
    ...
    if((n < B) && (m < M) && (h < H_out) && (w < W_out)) {
       ...
       if (c < C) {
           // int cur_c = c * K * K;
            ...
            share_partial_sum[idx] = acc;
            // atomicAdd(&y[cur_n_out + h * W_out + w], share_partial_sum[idx]);

       }
       else {
           share_partial_sum[idx] = 0.0;
       }

       // reduction sum
       for (unsigned int stride = blockDim.z / 2; stride >= 1; stride >>= 1){
           __syncthreads();
           if (c < stride)
               share_partial_sum[idx] += share_partial_sum[idx + stride * TILE_WIDTH * TILE_WIDTH];
       }
       __syncthreads();
       if (c == 0) {
           y[cur_n_out + h * W_out + w] = share_partial_sum[h0 * TILE_WIDTH + w0];
       }
...
// forward kernel with constant memory
__global__ void forward_shareInput_constKernel30_8(float *y, float *x)
{
    ...
    */
    ...
    // h_base = (blockIdx.z / W_grid) * TILE_WIDTH;
    // w_base = (blockIdx.z % W_grid) * TILE_WIDTH;
    h_base = (blockIdx.z / 4) * 8;
    w_base = (blockIdx.z % 4) * 8;
    // h_base = blockIdx.z * 8;
    // w_base = blockIdx.z * 8;
    h = h_base + h0;
    w = w_base + w0;
    ...
    if (c < 6) {
        ...
        // int cur_c = (c << 2) + (c << 7) + (c << 8) + (c << 9);
        // int int cur_share_c = c * X_tile_width * X_tile_width;
        int cur_share_c = c * 144;
        // int cur_share_c = (c << 4) + (c << 8) + (c << 10);
        for (int i = h0; i < 12; i += 8) {
            // int tmp1 = cur_share_c + (i << 5) + (i << 2);
            // int tmp = i + h_base;
            // int tmp2 = cur_c + (tmp << 1) + (tmp << 2) + (tmp << 3) + (tmp << 4) + w_base;
            int tmp1 = cur_share_c + i * 12;
            int tmp2 = cur_c + (i + h_base) * 30 + w_base;
            for (int j = w0; j < 12; j += 8) {
                // cur_i = cur_share_c + i * X_tile_width + j;
                cur_i = tmp1 + j;
                if ((i + h_base < 30) && (j + w_base < 30)) {
                    // x_cur_i = cur_c + (i + h_base) * W + j + w_base;
    ...
    int cur_n_out = n * 10816 + m * 676;
    // int cur_n_out = (n << 6) + (n << 9) + (n << 11) + (n << 13) + (m << 2) + (m << 5) + (m << 7) + (m << 9);
    __shared__ float share_par_sum[512];
    int idx = c * 64 + h0 * 8 + w0;
    ...
    if((n < 10000) && (m < 16) && (h < 26) && (w < 26)) {
       ...
       // for(int c = 0; c < 6; c++) {
        if (c < 6) {
           ...
           for(int p = 0; p < 5; p++) {
               ...
               // int tmp1 = cur_share_c + (tmp << 5) + (tmp << 2) + w0;
               // int tmp2 = cur_m + cur_c + (p << 2) + p;
               int tmp1 = cur_share_c + (h0 + p) * 12 + w0;
               int tmp2 = cur_m + cur_c + p * 5;
               for(int q = 0; q < 5; q++) {
                   cur_x = share_x[tmp1 + q];
                   // cur_x = *(share_x + cur_share_c + (h0 + p) * (X_tile_width) + (w0 + q));
           ...
           share_par_sum[idx] = acc;
       }
       else {
           share_par_sum[idx] = 0.0;
       }
       ...
       for (unsigned int stride = blockDim.z / 2; stride >= 1; stride >>= 1){
           __syncthreads();
           if (c < stride)
               share_par_sum[idx] += share_par_sum[idx + stride * 64];
       }
       __syncthreads();

       if (c == 0) {
           y[cur_n_out + h * 26 + w] = share_par_sum[h0 * 8 + w0];
       }
       // y[cur_n_out + (h << 1) + (h << 3) + (h << 4) + w] = acc;
       // y[cur_n_out + h * 26 + w] = acc;
       // *(y + cur_n_out + (h) * (W_out) + w) = acc;
                         ...
                         const mshadow::Tensor<gpu, 4, float> &w)
{
    ...
    // 8. input channel reduction with Atomic operation
    // dim3 blockDim2(8, 8, 8);
    // W_grid = ceil(1.0 * W_out / 8);
    // H_grid = ceil(1.0 * H_out / 8);
    // dim3 gridDim2(B, M, W_grid * H_grid);
    // forward_shareInput_constKernel_inputRedAtomic<<<gridDim2, blockDim2>>>(y.dptr_, x.dptr_, B, M, C, H, W, K);

    // 9. input channel reduction with tree
    // forward_shareInput_constKernel_inputRedTree<<<gridDim2, blockDim2>>>(y.dptr_, x.dptr_, B, M, C, H, W, K);

    // 10. fixed value for reduction tree
    // if (M == 6) {
    //     forward_shareInput_constKernel64<<<gridDim, blockDim>>>(y.dptr_, x.dptr_);
    // }
    // if (M == 16) {
    //     dim3 blockDim2(8, 8, 8);
    //     dim3 gridDim2(B, M, 16);
    //     // forward_shareInput_constKernel30<<<gridDim, blockDim>>>(y.dptr_, x.dptr_);
    //     forward_shareInput_constKernel30_8<<<gridDim2, blockDim2>>>(y.dptr_, x.dptr_);
    // }
